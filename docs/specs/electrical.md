...
> Middle section pins of the breadboards are not used for either power distribution or signal I/O.
...
!!! note "Power Quality and Regulation"
    ...
    **Do not use the *V5+ rail* as a voltage reference.** Instead, derive a local regulated supply using an on-board regulator. The *V5+ rail* is intended for powering digital logic and LED indicators only.
